...
[Route("api/trips")]
public class ApiTripsController : MyControllerBase
{
    private readonly ITripService tripService;

    public ApiTripsController(ITripService tripService)
    {
    ...
    public async Task<IActionResult> Review(int id, [FromBody] TripReviewRequest request)
    {
        var identity = HttpContext.User.Identity as ClaimsIdentity;
        var userId = int.Parse(identity.Claims.ElementAt(2).Value);
        var dto = await tripService.Review(id, userId, request.Content);
        if (dto.RespCode != System.Net.HttpStatusCode.Created)
        {
            if (dto.RespCode == System.Net.HttpStatusCode.NotFound)
            {
